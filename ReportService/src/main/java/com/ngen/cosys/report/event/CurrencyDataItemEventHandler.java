/**
 * Currency Data Item Event Handler
 * 
 ...
 * Currency Data Item Event Handler
 */
public class CurrencyDataItemEventHandler extends DataItemEventAdapter {
	...
	public void onRender(IDataItemInstance data, IReportContext reportContext) throws ScriptException {
		...
		if (Objects.nonNull(value) && Objects.nonNull(tenantRequest)
			...
			String format = tenantRequest.getTenantConfig().getFormat().getCurrencyFormat();
			
			DecimalFormat df = formatCache.get(Objects.isNull(format) ? ReportConstants.DEFAULT_NUMBER_FORMAT : format);
			// If Non Cached
			if (Objects.isNull(df)) {
				df = tenantRequest.getTenantConfig().getFormat().getCurrencyFormatter();
			...
			// Set Display Value
			String currencySymbol = tenantRequest.getTenantConfig().getFormat().getCurrencySymbol() != null ? tenantRequest.getTenantConfig().getFormat().getCurrencySymbol() : "SGD";
