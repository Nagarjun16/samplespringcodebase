/**
 * {@link ImportExportReferenceWorkingListReport}
 * 
...
import com.ngen.cosys.report.service.poi.model.CIQImportExportReferenceWorkingListReport;
...
/**
 * ImportExportReferenceReport is an implementation of Excel Report Service
 * 
 * @author NIIT Technologies Ltd
 */
@Component(value = ReportPOIConstants.IMPORT_EXPORT_REFERENCE_WORKINGLIST_REPORT)
public class ImportExportReferenceWorkingListReport extends POIBase implements ExcelReportService {

   private static final Logger LOGGER = LoggerFactory.getLogger(ImportExportReferenceWorkingListReport.class);
   ...
   private void populateReportData(POIElements elements, Object dataset) {
      List<CIQImportExportReferenceWorkingListReport> reportData = (List<CIQImportExportReferenceWorkingListReport>) dataset;
      Map<String, Long> groupByIndex = groupByCounts(reportData);
      int headerSize = ReportImportExportReferenceWorkingListConstants.HEADER_DETAILS.length;
      ...
      //
      for (CIQImportExportReferenceWorkingListReport report : reportData) {
         ...
         if (applyFormulaInNextRow) {
            ...
            for (int j = 0; j < headerSize; j++) {
               if (j == 4) {
                  ...
                  if (carrierGroupFormulaExclusion(j)) {
                     continue;
                  }
                  formula = elements.getFormulas().get(ReportPOIConstants.PERCENT);
                  formula = manipulateFormula(formula, "AN", "AM", String.valueOf(rowIndex - 1), false);
...
//         if (Objects.nonNull(report.getExportAWBMWBs())) { // && Objects.nonNull(report.getRcsSent())
            ...
            formula = manipulateFormula(formula, "G", "K", String.valueOf(rowIndex), false);
            cell = POIUtils.createCell(row, cellStyle, true, formula, false, columnIndex++, false, null, null);
            formula = elements.getFormulas().get(ReportPOIConstants.PERCENT);
            formula = manipulateFormula(formula, "K", "G", String.valueOf(rowIndex), false);
            cell = POIUtils.createCell(row, cellStyle, true, formula, false, columnIndex++, false, null, null);
//         }
         // 2nd segment
...
//         if (Objects.nonNull(report.getExportShipmentsManifested())) { // && Objects.nonNull(report.getDepSent()) && Objects.nonNull(report.getDepSentInTime())
            formula = elements.getFormulas().get(ReportPOIConstants.SUBTRACT);
            formula = manipulateFormula(formula, "H", "O", String.valueOf(rowIndex), false);
            cell = POIUtils.createCell(row, cellStyle, true, formula, false, columnIndex++, false, null, null);
            //
            formula = elements.getFormulas().get(ReportPOIConstants.SUBTRACT);
            ...
            //
            formula = elements.getFormulas().get(ReportPOIConstants.PERCENT);
            formula = manipulateFormula(formula, "P", "H", String.valueOf(rowIndex), false);
            cell = POIUtils.createCell(row, cellStyle, true, formula, false, columnIndex++, false, null, null);
//         }
         // 3rd segment
...
//         }
         // 5th segment
         cell = POIUtils.createCell(row, null, false, null, false, columnIndex++, true, 
               report.getNfdMessagesSent(), null);
...
//         if (Objects.nonNull(report.getDlvEvent()) && Objects.nonNull(report.getDlvSentInTime())) {
            ...
            cell = POIUtils.createCell(row, cellStyle, true, formula, false, columnIndex++, false, null, null);
            //
            formula = elements.getFormulas().get(ReportPOIConstants.PERCENT_DLV);
...
//         }
      ...
      if (applyFormulaInNextRow) {
         ...
         for (int j = 0; j < headerSize; j++) {
            if (j == 4) {
               String groupValue = ReportPOIConstants.GROUP_TOTAL.replace("{}", carrierGroup);
               cell = POIUtils.createCell(row, null, false, null, false, columnIndex++, false, null, groupValue);
            } else if ((j >= 0 && j < 4) || j == 5 || j == 41) { // Empty cell
               cell = POIUtils.createCell(row, null, false, null, false, columnIndex++, false, null, null);
            } else if (j == headerSize - 1) {
               formula = elements.getFormulas().get(ReportPOIConstants.PERCENT_DLV);
               formula = manipulateFormula(formula, "AN", "AM", String.valueOf(rowIndex - 1), false);
    ...
    */
   @Deprecated
   private boolean isHeaderEmpty(int columnIndex) {
      return StringUtils.isEmpty(ReportImportExportReferenceWorkingListConstants.HEADER_DETAILS[columnIndex]);
   }
    ...
    */
   @Deprecated
   private int[] getFormulaIndexes() {
      int[] formulaIndexes = null;
      String[] array = ReportImportExportReferenceWorkingListConstants.HEADER_DETAILS;
      if (ObjectUtils.isEmpty(array)) {
         int size = array.length - 1;
         formulaIndexes = new int[array.length];
         formulaIndexes = IntStream.range(0, size).filter(i -> array[i].isEmpty()).toArray();
      }
    ...
    * @return
    */
   private Map<String, Long> groupByCounts(List<CIQImportExportReferenceWorkingListReport> reportData) {
      return reportData.stream()
            .collect(Collectors.groupingBy(CIQImportExportReferenceWorkingListReport::getGroup, Collectors.counting()));
   }
   ...
   private void reportProperties(POIElements elements, ExcelReportRequest reportRequest) {
      ...
      elements.setFileName(ReportImportExportReferenceWorkingListConstants.IMPORT_EXPORT_REFERENCE_WORKINGLIST_FILE_NAME);
      ...
      String sheetName = String.format(ReportImportExportReferenceWorkingListConstants.IMPORT_EXPORT_REFERENCE_WORKINGLIST_SHEET_NAME,
   ...
   private void reportHeaderDetails(POIElements elements) {
      ...
      header.setHeaders(Arrays.asList(ReportImportExportReferenceWorkingListConstants.HEADER_DETAILS));
   ...
   private void reportFormulaDetails(POIElements elements) {
      ...
      formula.put(ReportPOIConstants.PERCENT_DLV, "(IF(OR(A$<>0,B$<>0),(A$/B$)*100,0))");
      elements.setFormulas(formula);
   }
   
   private boolean carrierGroupFormulaExclusion(int index) {
      return ReportImportExportReferenceWorkingListConstants.CARRIER_GROUP_FORMULA_EXCLUSION.contains(index);
   }
   ...
   private void reportDatasetDetails(ExcelReportRequest reportRequest, POIElements elements) {
      List<CIQImportExportReferenceWorkingListReport> reportData = sqlSession.selectList(
            ReportImportExportReferenceWorkingListConstants.SQL_SELECT_IMPORT_EXPORT_REFERENCE_WORKINGLIST_DATA,
            reportRequest);
