/**
 * Weight Data Item Event Handler
 * 
 ...
 * Weight Data Item Event Handler
 */
public class WeightDataItemEventHandler extends DataItemEventAdapter {
	...
	public void onRender(IDataItemInstance data, IReportContext reportContext) throws ScriptException {
		...
		if (Objects.nonNull(value) && Objects.nonNull(tenantRequest)
			...
			String format = tenantRequest.getTenantConfig().getFormat().getWeightFormat();
			
			DecimalFormat df = formatCache.get(Objects.isNull(format) ? ReportConstants.DEFAULT_NUMBER_FORMAT : format);
			// If Non Cached
			if (Objects.isNull(df)) {
				df = tenantRequest.getTenantConfig().getFormat().getWeightFormatter();
			...
			data.setDisplayValue(new DecimalFormat(ReportConstants.DEFAULT_NUMBER_FORMAT).format(0));
