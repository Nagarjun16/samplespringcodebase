 ...
 * Time Data Item Event Handler
 */
public class TimeDataItemEventHandler extends DataItemEventAdapter {
	//
	...
	public void onRender(IDataItemInstance data, IReportContext reportContext) throws ScriptException {
		...
		if (Objects.nonNull(value) && Objects.nonNull(tenantRequest)
			...
			String format = tenantRequest.getTenantConfig().getFormat().getTimeFormat();
			DateTimeFormatter df = formatCache
					.get(Objects.isNull(format) ? ReportConstants.DEFAULT_TIME_FORMAT : format);
			LocalDateTime dateTime = null;
			// If Non Cached
			if (Objects.isNull(df)) {
				df = tenantRequest.getTenantConfig().getFormat().getTimeFormatter();
			...
			if (value instanceof LocalDateTime) {
				...
				dateTime = ((LocalTime) value).atDate(LocalDate.now());
			...
			if (Objects.nonNull(dateTime)) {
				...
				if(!timeZoneConversion) {
					data.setDisplayValue(dateTime.format(df));
				} else {
				LocalTime time = TenantTimeZoneUtility.fromDatabaseToTenantDateTime(dateTime).toLocalTime();
				// Set Display Value
				data.setDisplayValue(time.format(df));
				}
			}
		} else {
