/**
 * This is an event component which post processes EAW shipment for inbound
 * process
 */
...
@Component(ImpBdEventTypes.Names.EAW_SHIPMENTS_EVENT)
public class EAWShipmentsEventStreamProcessorImpl implements BaseBusinessEventStreamProcessor {

   private static final Logger LOGGER = LoggerFactory.getLogger(EAWShipmentsEventStreamProcessorImpl.class);
   ...
   public void process(Message<?> payload) throws JsonGenerationException, JsonMappingException, IOException {
      ...
      EAWShipmentsEvent event = (EAWShipmentsEvent) convertJSONToObject.convertMapToObject(impBdPayload.getPayload(),
            EAWShipmentsEvent.class);

      // Get list of shipments whose SHC handling group belongs to EAP
      try {
         List<InboundShipmentInfoModel> customerShipmentList = this.eventService.getEAWShipments(event);
