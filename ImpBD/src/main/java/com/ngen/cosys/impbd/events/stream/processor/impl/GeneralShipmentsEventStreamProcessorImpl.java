/**
 * This is an event component which post processes General shipment for inbound
 * process
 */
...
@Component(ImpBdEventTypes.Names.GENEREAL_SHIPMENTS_EVENT)
public class GeneralShipmentsEventStreamProcessorImpl implements BaseBusinessEventStreamProcessor {

   private static final Logger LOGGER = LoggerFactory.getLogger(GeneralShipmentsEventStreamProcessorImpl.class);
   ...
   public void process(Message<?> payload) throws JsonGenerationException, JsonMappingException, IOException {
      ...
      GeneralShipmentsEvent event = (GeneralShipmentsEvent) convertJSONToObject
            .convertMapToObject(impBdPayload.getPayload(), GeneralShipmentsEvent.class);

      // Get list of shipments whose SHC handling group belongs to EAP
      try {
         List<InboundShipmentInfoModel> customerShipmentList = this.eventService.getGeneralShipments(event);
         ...
         LOGGER.error("Exception while fetching customer and their shipment list for EAW", customException);
      }
   }
