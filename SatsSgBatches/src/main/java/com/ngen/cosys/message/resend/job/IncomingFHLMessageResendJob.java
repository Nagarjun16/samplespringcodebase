/**
 * {@link IncomingFHLMessageResendJob}
 * 
...
/**
 * This implementation class is used for Incoming FHL Message Resends to APP 
 * for sequence message processing
 * 
 * @author NIIT Technologies Ltd
 */
@Component(value = MessageResendConstants.INCOMING_FHL_MESSAGE_RESEND)
public class IncomingFHLMessageResendJob extends AbstractCronJob implements MessageSequenceJob {

   private static final Logger LOGGER = LoggerFactory.getLogger(IncomingFHLMessageResendJob.class);
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      try {
         ...
         LOGGER.error("Incoming FHL Message Sequence Process exception occurred - {}", ex);
      }
   }
   ...
   public void messageSequenceReProcess() throws CustomException {
      ...
      LOGGER.info("Incoming FHL PROCESSING Message Logs Availability - {}, Size - {}",
      ...
      LOGGER.info("Incoming FHL Message PROCESSING Sequence Process Completed :: START TIME - {}, END TIME - {}",
   ...
   public void messageSequenceProcess() throws CustomException {
      LOGGER.info("Incoming FHL Message Sequence Process Initiated {}");
      LocalDateTime startTime = LocalDateTime.now();
      ...
      boolean messageLogsAvailability = !CollectionUtils.isEmpty(messageLogs) ? true : false;
      LOGGER.info("Incoming FHL INITIATED Message Logs Availability - {}, Size - {}",
            String.valueOf(messageLogsAvailability), messageLogs.size());
      if (messageLogsAvailability) {
         messageProcessor.process(messageLogs);
         logMessageSequenceProcessDetails(messageLogs);
      }
      LOGGER.info("Incoming FHL Message INITIATED Sequence Process Completed :: START TIME - {}, END TIME - {}",
            startTime, LocalDateTime.now());
   }
   ...
   public void logMessageSequenceProcessDetails(List<IncomingESBMessageLog> messageLogs) throws CustomException {
      LOGGER.info("Incoming FHL Message Sequence Process :: LOG Details - {}");
