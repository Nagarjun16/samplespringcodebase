/**
 * {@link IncomingCMDMessageResendJob}
 * 
...
/**
 * This implementation class is used for Incoming CMD Message Resends to APP 
 * for sequence message processing
 * 
 * @author NIIT Technologies Ltd
 */
@Component(value = MessageResendConstants.INCOMING_CMD_MESSAGE_RESEND)
public class IncomingCMDMessageResendJob extends AbstractCronJob implements MessageSequenceJob {

   private static final Logger LOGGER = LoggerFactory.getLogger(IncomingCMDMessageResendJob.class);
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      try {
         ...
         LOGGER.error("Incoming CMD Message Sequence Process exception occurred - {}", ex);
      }
   }
   ...
   public void messageSequenceReProcess() throws CustomException {
      ...
      LOGGER.info("Incoming CMD PROCESSING Message Logs Availability - {}, Size - {}",
      ...
      LOGGER.info("Incoming CMD Message PROCESSING Sequence Process Completed :: START TIME - {}, END TIME - {}",
   ...
   public void messageSequenceProcess() throws CustomException {
      LOGGER.info("Incoming CMD Message Sequence Process Initiated {}");
      LocalDateTime startTime = LocalDateTime.now();
      ...
      boolean messageLogsAvailability = !CollectionUtils.isEmpty(messageLogs) ? true : false;
      LOGGER.info("Incoming CMD INITIATED Message Logs Availability - {}, Size - {}",
            String.valueOf(messageLogsAvailability), messageLogs.size());
      if (messageLogsAvailability) {
         messageProcessor.process(messageLogs);
         logMessageSequenceProcessDetails(messageLogs);
      }
      LOGGER.info("Incoming CMD Message INITIATED Sequence Process Completed :: START TIME - {}, END TIME - {}",
            startTime, LocalDateTime.now());
   }
   ...
   public void logMessageSequenceProcessDetails(List<IncomingESBMessageLog> messageLogs) throws CustomException {
      LOGGER.info("Incoming CMD Message Sequence Process :: LOG Details - {}");
