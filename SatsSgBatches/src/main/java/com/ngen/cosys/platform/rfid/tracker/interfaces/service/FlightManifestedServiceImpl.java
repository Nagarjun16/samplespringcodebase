...
@Service
@Transactional
public class FlightManifestedServiceImpl implements FlightManifestedService {

	private static final Logger LOG = LoggerFactory.getLogger(FlightManifestedServiceImpl.class);
	@Autowired
	...
	public List<AwbModel> getFlightManifestedData(AuthModel auth) throws CustomException {
		...
		try {
			...
			LOG.debug("onPostExecute============  getFlightManifestedData" + searchFilterModels);

			SearchFilterModel searchFilterBO = SearchFilterUtils.getSearchFilter(searchFilterModels);

			LOG.debug("Filter============  getFlightManifestedData" + searchFilterBO);
			...
			if (searchFilterBO.getCarrierList().size() > 0) {
				awbsCarrShc = cosysRFIDMapperDAO.getFlightManifestedCarrShcData(searchFilterBO);
				...
				// LOG.debug("getFlightManifestedCarrShcData============ awbsCarrShc" +
				// awbsCarrShc.size());
			}
			if (searchFilterBO.getShcList().size() > 0) {
				awbsShc = cosysRFIDMapperDAO.getFlightManifestedShcData(searchFilterBO);
				...
				// LOG.debug("getFlightManifestedShcData============ awbsShc" + awbsShc.size());
			}
			awbs.addAll(awbBOs);
			awbs.addAll(awbShcBOs);
			// LOG.debug("getFlightManifestedData============ awbs" + awbs.size());
		} finally {
