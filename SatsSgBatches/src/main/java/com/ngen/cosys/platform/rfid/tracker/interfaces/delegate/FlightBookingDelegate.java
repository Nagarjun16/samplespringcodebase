...
public class FlightBookingDelegate {
	...
	private static final Logger LOG = LoggerFactory.getLogger(FlightBookingDelegate.class);

	@Value("${tracer.platform.flight-booking-endpoint}")
	private String flightBookingEndPoint;
	...
	public void getFlightBooking() {

		List<AwbModel> awbs = null;
		AuthModel auth = null;
		try {

			auth = postHttpUtilityService.getAuthUserPassword();

			awbs = flightBookingService.getFlightBookingData(auth);

			LOG.info(new Date() + "=== " + "flightBookingJob===========================After  " + awbs.size());
...
// 					if (awbBOs.size() > 100) {
						groupDataPush(awbBOs, 100, flightBookingEndPoint, auth, dateNow, lastRecFlag);
						awbBOs.clear();
//					}
				}
...
//						groupDataPush(awbBOs, 100, flightBookingEndPoint, auth, dateNow);
//						awbBOs.clear();
...
//				}
//
//				groupDataPush(awbBOs2, 100, flightBookingEndPoint, auth, dateNow);

				ApiRequestModel apiRequest = new ApiRequestModel();
				apiRequest.setAwbList(awbs);
				try {
					FormatUtils.handle(apiRequest, "BOOKING");
				} catch (IOException e) {
				...
				LOG.info("No Record Found for   flight Booking");
			}
...
//				apiRequest.setStation("SATS");
//				apiRequest.setFlightType("E");
				LOG.info("FlightBookingJob count == groupNumber ============  " + " " + "AwbType" + " "
...
//				apiRequest.setFromDate(TenantZoneTime.getZoneDateTime(dateNow.minusMinutes(searchFilterModel.getExportDataPushMinusFrom()), "SIN").toString());
//				apiRequest.setToDate(TenantZoneTime.getZoneDateTime(dateNow.plusMinutes(searchFilterModel.getExportDataPushPlusTo()), "SIN").toString());
//				apiRequest.setIsLastRecord("Y");
//				apiRequest.setStation("SATS");
//				apiRequest.setFlightType("E");
//				LOG.info("FlightBookingJob count == arraySize ============  " + " " + "AwbType" + " "
...
//			ApiRequestModel apiRequest = new ApiRequestModel();
//			apiRequest.setAwbList(subResult);
//			LOG.info("FlightBookingJob count != 0 ============  " + " " + "AwbType" + " " + subResult.get(0).getStage()
//					+ " " + subResult.size());
//			postHttpUtilityService.onPostExecute(apiRequest, endpoint, auth);
//		}
	}
