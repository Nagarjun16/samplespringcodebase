...
@Service
public class FlightManifestedDelegate {

	private static final Logger LOG = LoggerFactory.getLogger(FlightManifestedDelegate.class);

	@Value("${tracer.platform.flight-manifested-endpoint}")
	private String flightManifestEndPoint;
	...
	@Autowired
	private FlightManifestedService flightManifestedService;

	@Autowired
	private PostHttpUtilityService postHttpUtilityService;

	public void getFlightManifested() {
		List<AwbModel> awbs;
		AuthModel auth = null;
		try {
			...
			awbs = flightManifestedService.getFlightManifestedData(auth);

			LOG.info(new Date() + "=== " + "getFlightManifestedData===========================After  " + awbs.size());
...
// 					if (awbBOs.size() > 100) {
						groupDataPush(awbBOs, 100, flightManifestEndPoint, auth, dateNow, lastRecFlag);
						awbBOs.clear();
//					}
				}
...
//						groupDataPush(awbBOs, 100, flightManifestEndPoint, auth);
//						awbBOs.clear();
...
//				}
//
//				groupDataPush(awbBOs2, 100, flightManifestEndPoint, auth);

				ApiRequestModel apiRequest = new ApiRequestModel();
				apiRequest.setAwbList(awbs);
				try {
					FormatUtils.handle(apiRequest, "MANIFEST");
				} catch (IOException e) {
...
//			if (count == groupNumber) {
				...
				apiRequest.setFromDate(TenantZoneTime.getZoneDateTime(dateNow.minusMinutes(searchFilterModel.getExportDataPushMinusFrom()),searchFilterModel.getTenantId()).toString());
				apiRequest.setToDate(TenantZoneTime.getZoneDateTime(dateNow.plusMinutes(searchFilterModel.getExportDataPushPlusTo()),searchFilterModel.getTenantId()).toString());
//				if(awbs2.size() == groupNumber) {
					apiRequest.setIsLastRecord(lastRecFlag);
//				}
				apiRequest.setStation("SATS");
				apiRequest.setFlightType("E");
//				apiRequest.setFromDate(TenantZoneTime.getZoneDateTime(LocalDateTime.now().minusMinutes(searchFilterModel.getExportDataPushMinusFrom()),searchFilterModel.getTenantId()).toString());
//				apiRequest.setToDate(TenantZoneTime.getZoneDateTime(LocalDateTime.now().plusMinutes(searchFilterModel.getExportDataPushPlusTo()),searchFilterModel.getTenantId()).toString());
//				if(awbs2.size() == groupNumber) {
//					apiRequest.setIsLastRecord("Y");
//				}
//				apiRequest.setStation("SATS");
//				apiRequest.setFlightType("E");
				LOG.info("FlightManifestedJob count == groupNumber ============  " + " " + "AwbType" + " "
...
//				apiRequest.setFromDate(TenantZoneTime.getZoneDateTime(LocalDateTime.now().minusMinutes(searchFilterModel.getExportDataPushMinusFrom()), "SIN").toString());
//				apiRequest.setToDate(TenantZoneTime.getZoneDateTime(LocalDateTime.now().plusMinutes(searchFilterModel.getExportDataPushPlusTo()), "SIN").toString());
//				apiRequest.setIsLastRecord("Y");
//				apiRequest.setStation("SATS");
//				apiRequest.setFlightType("E");
//				LOG.info("FlightManifestedJob count == arraySize ============  " + " " + "AwbType" + " "
...
//			ApiRequestModel apiRequest = new ApiRequestModel();
//			apiRequest.setAwbList(subResult);
//			LOG.info("FlightManifestedJob count != 0 ============  " + " " + "AwbType" + " "
//					+ subResult.get(0).getStage() + " " + subResult.size());
//			postHttpUtilityService.onPostExecute(apiRequest, endpoint, auth);
//		}
	}
