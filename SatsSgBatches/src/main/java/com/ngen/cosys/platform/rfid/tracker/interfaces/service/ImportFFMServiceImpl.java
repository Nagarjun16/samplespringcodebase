...
@Service
@Transactional
public class ImportFFMServiceImpl implements ImportFFMService {

	private static final Logger LOG = LoggerFactory.getLogger(ImportFFMServiceImpl.class);
	...
	public List<AwbModel> getImportFFMData(AuthModel auth) throws CustomException {
		...
		try {
			...
			LOG.debug("onPostExecute============  getImportFFMData" + searchFilterModels);

			SearchFilterModel searchFilterBO = SearchFilterUtils.getSearchFilter(searchFilterModels);

			LOG.debug("Filter============  getImportFFMData" + searchFilterBO);

			if (null == searchFilterBO) {
				searchFilterBO = new SearchFilterModel();
			}

			LOG.debug("getImportFFMData============  searchFilterBO" + searchFilterBO);
			if (searchFilterBO.getCarrierList().size() > 0) {
				
				if (searchFilterBO.getCarrierList().size() > 0) {
					awbsCarrShc.addAll(cosysRFIDMapperDAO.getImportFFMCarrShcData(searchFilterBO));
				}
				...
				for (int i = 0; i < dupList.size(); i++) {
					...
					for (int i1 = 0; i1 < awbsCarrShc.size(); i1++) {
						...
						if (dupList.get(i).contains((obj.getFlightKeyDate()) + obj.getAwbNo())) {
							...
							for (int j = 0; j < searchFilterBO.getSatsCarShcList().size(); j++) {

								String filter = searchFilterBO.getSatsCarShcList().get(j);
				...
				// LOG.debug("getImportFFMCarrShcData============ awbsCarrShc" +
				// awbsCarrShc.size());
			}

			if (searchFilterBO.getSatsShcList().size() > 0 || searchFilterBO.getSatsShcList().size() > 0) {
				
				if (searchFilterBO.getShcList().size() > 0) {
					awbsShc.addAll(cosysRFIDMapperDAO.getImportFFMShcData(searchFilterBO));
				}
				...
				for (int i = 0; i < dupList.size(); i++) {
					...
					for (int i1 = 0; i1 < awbsShc.size(); i1++) {
						...
						if (dupList.get(i).contains((obj.getFlightKeyDate()) + obj.getAwbNo())) {
							...
							for (int j = 0; j < searchFilterBO.getShcList().size(); j++) {

								String filter = searchFilterBO.getShcList().get(j);
								String shc = filter.substring(0, 3);
				...
				// LOG.debug("getImportFFMShcData============ awbsShc" + awbsShc.size());
			}
			awbs.addAll(awbBOs);
			awbs.addAll(awbShcBOs);
			// LOG.debug("getImportFFMData============ awbs" + awbs.size());
		} finally {
