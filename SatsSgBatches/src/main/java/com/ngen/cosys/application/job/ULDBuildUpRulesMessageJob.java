...
public class ULDBuildUpRulesMessageJob extends AbstractCronJob {
   @Autowired
   ...
   private static final String messageName = "TGFMUCRL";
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      ...
      MssMessageParentModel<List<ULDBuildupRulesModel>> uldBUildUp = null;
      try {
         uldBUildUp = service.uldBuildUPRulesMessages(messageName);
      } catch (CustomException e) {
         throw new JobExecutionException(e.getCause());
      }
      // Send batch job data to Connector framework
      if (!CollectionUtils.isEmpty((List) uldBUildUp.getData())) {

         delegateToConnector(uldBUildUp, messageName);
      }
   }
