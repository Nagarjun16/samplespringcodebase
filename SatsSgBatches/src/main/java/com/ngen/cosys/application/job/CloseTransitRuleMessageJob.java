...
public class CloseTransitRuleMessageJob extends AbstractCronJob {
   @Autowired
   ...
   private static final String messageName = "TGFMCLTL";
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      ...
      MssMessageParentModel<List<CloseTransitMessagesModel>> closeTransit = null;
      try {
         closeTransit = service.closeTransitRuleMessages(messageName);
      } catch (CustomException e) {
         throw new JobExecutionException(e.getCause());
      }
      // Send batch job data to Connector framework
      if (!CollectionUtils.isEmpty((List)closeTransit.getData())) {
         delegateToConnector(closeTransit, messageName);
      }
   }
