...
public class OuthouseTransferRulesMessagesjob extends AbstractCronJob {
   @Autowired
   ...
   private static final String messageName = "TGFMTRML";
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      ...
      MssMessageParentModel<List<Object>> outhouseDataList = null;
      try {
         outhouseDataList = service.outhouseTransferRulesMessages(messageName);
      } catch (CustomException e) {
         throw new JobExecutionException(e.getCause());
      }
      // Send batch job data to Connector framework
      if (!CollectionUtils.isEmpty((List) outhouseDataList.getData())) {
         delegateToConnector(outhouseDataList, messageName);
      }
   }
