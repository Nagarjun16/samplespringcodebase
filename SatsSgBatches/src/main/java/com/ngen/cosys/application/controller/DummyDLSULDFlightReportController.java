...
@NgenCosysAppInfraAnnotation(path = "/api/dlsuld/flightreport")
public class DummyDLSULDFlightReportController {
   private static final Logger LOGGER = LoggerFactory.getLogger(FFMFWBMonitoringController.class);
   ...
   @PostMapping(value = "/api/batches/dlsuldflightreport", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
   public void dlsuldflightreport() throws CustomException {
     ...
     // Report Payload
        ...
        LOGGER.info("DLS ULD Flight Report  Job :: DLS ULD Flight Report sent successfully");
     }
  ...
  private EMailEvent getEmailPayload(List<String> emailAddresses, LocalDateTime currentZoneTime) {
     ...
     String fileName = null;
        fileName = shiftFileNameFormat(DLS_ULD_FLIGHT_REPORT_NAME, currentZoneTime);
  ...
  private String shiftFileNameFormat(String reportName, LocalDateTime currentZoneTime) {
     return reportName; //+dateFormat(currentZoneTime)+currentZoneTime.format(DateTimeFormatter.ofPattern(ONLYTIME_FORMAT));
  }
  ...
  private ReportMailPayload getReportMailPayload(Map<String, Object> reportParameters, 
     ...
     ReportMailPayload reportMailPayload = new ReportMailPayload();
        ...
        reportMailPayload.setFileName(shiftFileNameFormat(DLS_ULD_FLIGHT_REPORT_NAME, currentZoneTime));
