...
public class XRayRulesMessageJob extends AbstractCronJob {
   @Autowired
   ...
   private static final String messageName = "TGFMXRYL";
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      ...
      MssMessageParentModel<List<XRayRulesMessagesModel>> xrayRules = null;
      try {
         xrayRules = service.xrayRuleMessages(messageName);
      } catch (CustomException e) {
         throw new JobExecutionException(e.getCause());
      }
      // Send batch job data to Connector framework
      if (!CollectionUtils.isEmpty((List) xrayRules.getData())) {
         delegateToConnector(xrayRules, messageName);
      }
   }
