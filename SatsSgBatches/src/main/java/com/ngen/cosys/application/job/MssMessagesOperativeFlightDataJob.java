...
public class MssMessagesOperativeFlightDataJob extends AbstractCronJob {
   @Autowired
   ...
   private static final String messageName = "TGFMOUFL";
   ...
   protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
      ...
      MssMessageParentModel<List<MssModelOperativeFlightData>> flightData = null;
      try {
         flightData = service.operativeFlightData(messageName);
      } catch (CustomException e) {
         throw new JobExecutionException(e.getCause());
      }
      // Send batch job data to Connector framework
      if (!CollectionUtils.isEmpty((List)flightData.getData())) {
         delegateToConnector(flightData, messageName);
      }
