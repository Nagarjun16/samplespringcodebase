...
import com.ngen.cosys.message.resend.job.ResendCAMSMessagesJob;
import com.ngen.cosys.message.resend.service.ResendCAMSMessagesService;

import io.swagger.annotations.ApiOperation;

@RestController
public class ResendCAMSMessageController {
   ...
   @RequestMapping(value = "/api/dummy/resendcamsmsg", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
   public void resendCamsMsg() throws CustomException {
            ...
            if (event.getRetryCount() == event.getConfiguredRetryCount()) {
               ...
               if (event.isLongSleepFalg()) {
                  ...
                  if (event.getRetryCount() > 1) {
                     // long sleep
                     if (event.getTimeDiffFromLastUpdated() >= event.getLongsleepTime()
                           && event.getRetryCount() < event.getConfiguredRetryCount()) {
